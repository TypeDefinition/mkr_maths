...
namespace mkr {
    ...
    vector3 plane::closest_point(const vector3 &_point) const {
        ...
        const float lambda = (-d_ - normal_.dot(_point)) / normal_.length_squared();
    ...
    std::optional<vector3> plane::intersect_point(const line &_line) const {
        ...
        const float lambda = (-d_ - normal_.dot(_line.point_)) / normal_.dot(_line.direction_);
    ...
    std::optional<line> plane::intersect_line(const plane &_plane) const {
         ...
         * Let plane c be a plane whose normal is the cross product of plane 1's normal & plane 2's normal, and passes through the origin.
         */
        vector3 plane_c_normal = normal_.cross(_plane.normal_);
        const float determinant = plane_c_normal.length_squared();
